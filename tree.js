var coordSystem = function (b) {
  ...
  createBranch = function (b, g, r, w, h, l, v, n, x) {
    ...
    return {
      branch: BABYLON.MeshBuilder.CreateRibbon(
        "branch",
        { pathArray: t, closeArray: !0 },
        x
      ),
      core: c,
      _radii: q,
  ...
  createTreeBase = function (b, g, r, w, h, l, v, n, x, t) {
    ...
    A.push(c);
    for (
      var q = y[y.length - 1], y = (2 * Math.PI) / h, z, u, p, C, B = 0;
      B < h;
      B++
    )
      if (
        ...
        for (var D = 0; D < h; D++)
          (u = randPct(D * y, 0.25)),
            ...
            A.push(u);
    return {
      tree: BABYLON.Mesh.MergeMeshes(a),
      paths: m,
      radii: e,
      directions: A,
    };
  ...
  createTree = function (b, g, r, w, h, l, v, n, x, t, d, c, f, q, a, m) {
    ...
    b.addShape(n, 2 * f * Math.pow(l, h), {
      positionFunction: function (b, a, g) {
        ...
        b.position = new BABYLON.Vector3(
          e.paths[a][k].x +
            ((e.paths[a][d].x - e.paths[a][k].x) * (E - c)) / (m - c),
          e.paths[a][k].y +
            ((e.paths[a][d].y - e.paths[a][k].y) * (E - c)) / (m - c) +
            ((0.6 * z) / q + e.radii[a][d]) * ((g % 2) * 2 - 1),
          e.paths[a][k].z +
            ((e.paths[a][d].z - e.paths[a][k].z) * (E - c)) / (m - c)
        );
    ...
    for (
      var C = [], B = [], p = e.paths.length, D = e.paths[0].length, F = 0;
      F < x;
      F++
    )
